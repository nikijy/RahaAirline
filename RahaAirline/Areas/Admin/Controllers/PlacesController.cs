using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Net;
using System.Web;
using System.Web.Mvc;
using DataLayer;
using System.IO;

namespace RahaAirline.Areas.Admin.Controllers
{
    [Authorize(Users = "admin@example.com")]
    public class PlacesController : Controller
    {
        private IPlaceRepository placeRepository;
        RahaAirlineContext db = new RahaAirlineContext();
        public PlacesController()
        {
            placeRepository = new PlaceRepository(db);
        }

        // GET: Admin/Places
        public ActionResult Index()
        {
            return View(placeRepository.GetAllPlaces());
        }

        // GET: Admin/Places/Details/5
        public ActionResult Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Place place = placeRepository.GetPlaceById(id.Value);
            if (place == null)
            {
                return HttpNotFound();
            }
            return View(place);
        }

        // GET: Admin/Places/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: Admin/Places/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "PlaceID,Country,City,Image,Price,Visit")] Place place, HttpPostedFileBase imgPlace)
        {
            if (ModelState.IsValid)
            {
                place.Visit = 0;
                if (imgPlace != null)
                {
                    place.Image = Guid.NewGuid() + Path.GetExtension(imgPlace.FileName);
                    imgPlace.SaveAs(Server.MapPath("/PlaceImages/" + place.Image));
                }

                placeRepository.InsertPlace(place);
                placeRepository.Save();
                return RedirectToAction("Index");
            }

            return View(place);
        }

        // GET: Admin/Places/Edit/5
        public ActionResult Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Place place = placeRepository.GetPlaceById(id.Value);
            if (place == null)
            {
                return HttpNotFound();
            }
            return View(place);
        }

        // POST: Admin/Places/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "PlaceID,Country,City,Image,Price,Visit")] Place place, HttpPostedFileBase imgPlace)
        {
            if (ModelState.IsValid)
            {
                if (imgPlace != null)
                {
                    if (place.Image != null)
                    {
                        System.IO.File.Delete(Server.MapPath("/PlaceImages/" + place.Image));
                    }


                    place.Image = Guid.NewGuid() + Path.GetExtension(imgPlace.FileName);
                    imgPlace.SaveAs(Server.MapPath("/PlaceImages/" + place.Image));
                }

                placeRepository.UpdatePlace(place);
                placeRepository.Save();
                return RedirectToAction("Index");
            }
            return View(place);
        }

        // GET: Admin/Places/Delete/5
        public ActionResult Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Place place = placeRepository.GetPlaceById(id.Value);
            if (place == null)
            {
                return HttpNotFound();
            }
            return View(place);
        }

        // POST: Admin/Places/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            var place = placeRepository.GetPlaceById(id);
            if (place.Image != null)
            {
                System.IO.File.Delete(Server.MapPath("/PlaceImages/" + place.Image));
            }
            placeRepository.DeletePlace(id);
            placeRepository.Save();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                placeRepository.Dispose();
                db.Dispose();
            }
            base.Dispose(disposing);
        }
    }
}
